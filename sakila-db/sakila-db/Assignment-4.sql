...
-- Q4: Show all customers and staff emails from both customer and staff tables using a full outer join 
-- (simulate using LEFT + RIGHT + UNION).
SELECT c.customer_id, c.email AS customer_email, 
       s.staff_id, s.email AS staff_email
FROM customer c
LEFT JOIN staff s ON c.customer_id = s.staff_id
...
SELECT c.customer_id, c.email AS customer_email, 
       s.staff_id, s.email AS staff_email
FROM customer c
RIGHT JOIN staff s ON c.customer_id = s.staff_id;
